...
def _infer_duration_from_timeline(timeline: list) -> str:
    max_seconds = 0
    ...
    for event in timeline:
        ...
        end_time = event.get("fi")
        if isinstance(end_time, str):
            max_seconds = max(max_seconds, _mmss_to_seconds(end_time))

    return _seconds_to_mmss(max_seconds)
...
def analyze_video(
    ...
    if parsed.get("timeline"):
        parsed["combat_info"]["durada_estimada"] = _infer_duration_from_timeline(
            parsed["timeline"]
        )
